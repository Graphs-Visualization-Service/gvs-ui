 ...
 *
 */
public class GraphSessionController implements Observer, IGraphSessionController {
	 ...
	 * @param pGraphModels
	 */
	public GraphSessionController(long pSessionId, String pSessionName, Vector pGraphModels) {
	...
	public GraphSessionController(long pSessionId, String pSessionName, GraphModel pGraphModel) {
		try {
		...
		if ((actualGraphModel.getVertizes()).size() != 0) {
			if (((IVertex) actualGraphModel.getVertizes().firstElement()).isRelative()) {
	...
	private void initializeGraphSessionController() {
		...
		this.graphContLogger = LoggerFactory.getLogger(GraphSessionController.class);
	...
	public void update(Observable o, Object arg) {
		...
		if (parameter.equals("TRUE")) {
			...
			graphContLogger.debug("Continue layouting graph, vertizes without positions detected");
	...
	/**
	 * Validats if user interactions are allowed
	 */
	public boolean validateNavigation(long pRequestedModelId) {
		if ((pRequestedModelId >= 1) && (pRequestedModelId <= (graphModels.size()))) {
	...
	public void isDraggable() {
		if (actualGraphModel.getModelId() == graphModels.size()) {
			...
			graphContLogger.debug("Graph isn't last element in queue, disable dragging");
			visualModel.setDragging(false);
	...
	public synchronized void getFirstModel() {
		...
		int requestedModelId = ((GraphModel) graphModels.firstElement()).getModelId();
	...
	public void getLastModel() {
		...
		int requestedModelId = ((GraphModel) graphModels.lastElement()).getModelId();
	...
	private void setFormerVertexCoordinate() {
		GraphModel formerModel = (GraphModel) graphModels.elementAt(actualGraphModel.getModelId() - 2);
		...
		while (it1.hasNext()) {
			...
			if (isFormerVertexPosAvailable == false) {
				callLayoutEngine = true;
			}
		}

		if (callLayoutEngine == true) {
			graphContLogger.debug("Vertizes without positions detected, call layouter");
			callLayouter();
	...
	private void setActualVertexCoordinate(IVertex pActualVertex, IVertex pFormerVertex) {
	...
	private void setButtonState(long pRequestedModelId) {
		if (replayMode == false && (autoLayoutingMode == false)) {
	...
	private void callLayouter() {
		...
		layoutController.setElements(actualGraphModel.getVertizes(), actualGraphModel.getEdges(),
