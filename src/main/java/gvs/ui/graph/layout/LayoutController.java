...
public class LayoutController extends Observable implements Observer,Tickable{
	...
	/**
	 *  Checks if particles in area are stable. If true, stops layouting engine, waits 1500ms
	 *  and displays with correct components
	 */
	public void tick(double rate, double rateRatio, boolean drop, long iteration, long time) {
	...
	public void setElements(Vector vertices, Vector edges, boolean doSoftLayout) {
		...
		graphContLogger.info("LayoutController has new elements detected, start layouting procedure");
	...
	public void createVertexParticles() {
		...
		while(it.hasNext()) {
			...
			particle = new Particle(new AreaPoint(p),myVertex.getId(),myVertex,myVertex.isFixedPosition(), 50/*masse*/,40/*radius*/);
	...
	private Point generateRandomPoints(IVertex vertex){
		...
		randomPoint.x=(int)((double)(area.getUniverseDimension().dimensionWidth())*Math.random());
		randomPoint.y=(int)((double)(area.getUniverseDimension().dimensionHeight())*Math.random());
		return randomPoint;
