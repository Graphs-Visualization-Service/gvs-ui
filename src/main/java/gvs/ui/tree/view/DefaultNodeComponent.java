...
public class DefaultNodeComponent extends JComponent {
	 ...
	 * @param pMaxPixel
	 */
	public DefaultNodeComponent(IBinaryNode pNode, Dimension pDim, int pLabelLength, int pMaxPixel) {
		super();
		this.node = pNode;
		this.dim = pDim;
	...
	private void drawBinaryNode(Graphics2D g2) {
		...
		g2.draw(new Ellipse2D.Double(xPosition - xlineOffset, yPosition - ylineOffset, rectWidthLine, rectHeightLine));

		g2.setPaint(vertexFillColor);
		g2.fill(new Ellipse2D.Double(xPosition - xEllipseOffset, yPosition - yEllipseOffset, rectWidth, rectHeight));
	}
	...
	private void drawChildEdges(Graphics2D g2) {
		if (node.getLeftChild() != null) {
			...
			g2.drawLine((int) xPosition, (int) yPosition,
					(int) (node.getLeftChild().getXPosition() * onePercentX) + BORDEROFFSET,
		...
		if (node.getRightChild() != null) {
			...
			g2.drawLine((int) xPosition, (int) yPosition,
					(int) (node.getRightChild().getXPosition() * onePercentX) + BORDEROFFSET,
	...
	/**
	 * Sets dimension, used for recalculating the position to resize
	 * frame
	 * 
	 * @param pDim
	 */
