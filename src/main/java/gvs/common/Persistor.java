...
public class Persistor {
	...
	private void saveGraphSession(Element pElement, GraphSessionController pSessionController) {
		Element eSession = pElement.addElement(GRAPH);
		eSession.addAttribute(ATTRIBUTEID, String.valueOf(pSessionController.getSessionId()));
	...
	private void saveTreeSession(Element pElement, TreeSessionController pSessionController) {
	...
	private void saveDefaultVertex(DefaultVertex pVertex, Element pVertizes) {
		...
		eLineThick.addText(configuration.getLineThicknessName((int) stroke.getLineWidth()));
	...
	private TreeSessionController loadTreeSession(Element pTreeSession) {
		...
		while (modelIt.hasNext()) {
			...
			if (eTreeModel.getName() == TREEMODEL) {
				...
				treeModels.add(new TreeModel(treeLabel, modelId, Integer.parseInt(maxLabelLength), Color.WHITE,
						rootNode, nodes));
			}
		}
	...
	private IVertex loadDefaultVertex(Element pVertex) {
		...
		BasicStroke lineStroke = configuration.getLineObject(linestyle, linethickness);
		...
		if (eIcon != null) {
			...
			return new DefaultVertex(vertexId, label, lineColor, lineStroke, fillColor, xpos, ypos, false);
		} else {
			return null;
		}
	}
	...
	private INode loadBinaryNode(Element pNode) {
		...
		return new BinaryNode(nodeId, label, lineColor, lineStroke, fillColor, leftchildId, rigthchildId, xPos, yPos);
	}
}
