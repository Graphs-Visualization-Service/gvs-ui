...
public class Area extends Observable {
	 ...
	 * overwriting default dimension and viscosity. Viscosity has an influence
	 * on how fast accelerated particles slow down
	 * 
	 * @param dimension
	 * @param viscosity
	 */
	...
	/**
	 * Each time the are ticker sends a pulse, updateAll method will be called.
	 * So particles are accelerated into claimed direction until they are stable
	 *
	 */
	public synchronized void updateAll() {
		...
		while (it.hasNext()) {
			Particle p = (Particle) it.next();
			AreaVector vectorToCentre = new AreaVector(p.getPointPosition(), getAreaCenter());
