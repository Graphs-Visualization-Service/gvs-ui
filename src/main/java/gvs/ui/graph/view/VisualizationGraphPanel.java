 ...
 *
 */
public class VisualizationGraphPanel extends JPanel
		implements Observer, MouseListener, MouseMotionListener, IVisualizationGraphPanel {
	...
	public VisualizationGraphPanel(VisualizationGraphModel visualModel) {
		...
		this.graphContLogger = LoggerFactory.getLogger(VisualizationGraphPanel.class);
	...
	private void createVertexGraphComponents(Vector pVertizes, Dimension pDimension) {
		...
		while (verIt.hasNext()) {
			...
			for (int i = 0; i < interfaces.length; i++) {
				if (interfaces[i] == IDefaultVertex.class) {
					...
					IVertexComponent nodeComponent = new DefaultVertexComponent((IDefaultVertex) vertex, pDimension,
							effectiveLabelLength, effectiveLabelPixel);
					nodeComponents.add(nodeComponent);
				} else {
					graphContLogger.debug("Creating icon vertex");
					IVertexComponent iconComponent = new IconVertexComponent((IIconVertex) vertex, pDimension,
							effectiveLabelLength, effectiveLabelPixel);
	...
	private void createEdgeGraphComponents(Vector pEdges, Dimension dimension) {
		...
		while (edgIt.hasNext()) {
			...
			edgeComponent = new EdgeComponent(edge, dimension, backgroundColor, check, effectiveLabelPixel);
	...
	public void update(Observable o, Object arg) {
		Dimension dimension = new Dimension((this.getWidth() / HUNDREDPERCENT), (this.getHeight() / HUNDREDPERCENT));
	...
	public void paint(Graphics g) {
		...
		if (visualModel.isLayouting() == false) {
			...
			while (verL.hasNext()) {
				...
				int x = (int) (temp.getXPosition() * onePercentX), y = (int) (temp.getYPosition() * onePercentY);
	...
	public void mousePressed(MouseEvent evt) {
		...
		try {
			...
			while (it.hasNext()) {
				...
				if ((Math.abs(evt.getX() - n.getXPosition()) < 20) && (Math.abs(evt.getY() - n.getYPosition()) < 20)) {
