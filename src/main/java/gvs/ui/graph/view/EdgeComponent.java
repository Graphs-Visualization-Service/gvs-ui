...
public class EdgeComponent extends JComponent {
	 ...
	 * @param pPixelLength
	 */
	public EdgeComponent(IEdge pEdge, Dimension pDim, Color panelColor, LabelConflictCheck check, int pPixelLength) {
		super();
		...
		this.startVertexXPos = startVertex.getXPosition() * (double) onePercentX + BOUNDARYXOFFSET;
		this.startVertexYPos = startVertex.getYPosition() * (double) onePercentY + BOUNDARYYOFFSET;
		this.endVertexXPos = endVertex.getXPosition() * (double) onePercentX + BOUNDARYXOFFSET;
		this.endVertexYPos = endVertex.getYPosition() * (double) onePercentY + BOUNDARYYOFFSET;
	...
	private void labelEdge(Graphics2D g2) {
		...
		g2.fill(new RoundRectangle2D.Double(labelXPos, labelYPos, labelLenght, labelHeight, 0.0, 0.0));
	...
	private void calculateLength() {
		...
		double y = (ELLIPSEHEIGHT * ellipseOffset) / Math.sqrt(
				ellipseOffset * ellipseOffset + ELLIPSEHEIGHT * ELLIPSEHEIGHT * Math.tan(alpha) * Math.tan(alpha));
	...
	public void paint(Graphics g) {
		...
		if (isDirected) {
			if (startVertex != endVertex) {
				calculateLength();
				drawArrow(g2, (int) startVertexXPos, (int) startVertexYPos, (int) endVertexXPos, (int) endVertexYPos,
						1.0f);
			} else {
				g2.draw(new Line2D.Double(startVertexXPos, startVertexYPos - ELLIPSEHEIGHT, endVertexXPos,
						endVertexYPos - ELLIPSEHEIGHT * 2));
				g2.draw(new Line2D.Double(startVertexXPos, startVertexYPos - ELLIPSEHEIGHT * 2,
						endVertexXPos + ellipseOffset * 2, endVertexYPos - ELLIPSEHEIGHT * 2));
				g2.draw(new Line2D.Double(startVertexXPos + ellipseOffset * 2, startVertexYPos - ELLIPSEHEIGHT * 2,
						endVertexXPos + ellipseOffset * 2, endVertexYPos));
				drawArrow(g2, (int) (startVertexXPos + ellipseOffset * 2), (int) startVertexYPos,
						(int) (endVertexXPos + ellipseOffset), (int) endVertexYPos, 1.0f);
			}
		} else {
			g2.setStroke(lineStroke);
			if (startVertex != endVertex) {
				calculateLength();
				g2.draw(new Line2D.Double(startVertexXPos, startVertexYPos, endVertexXPos, endVertexYPos));
			} else {
				g2.draw(new Line2D.Double(startVertexXPos, startVertexYPos - ELLIPSEHEIGHT, endVertexXPos,
						endVertexYPos - ELLIPSEHEIGHT * 2));
				g2.draw(new Line2D.Double(startVertexXPos, startVertexYPos - ELLIPSEHEIGHT * 2,
				...
				g2.draw(new Line2D.Double(startVertexXPos + ellipseOffset * 2, startVertexYPos - ELLIPSEHEIGHT * 2,
						endVertexXPos + ellipseOffset * 2, endVertexYPos));
				g2.draw(new Line2D.Double(startVertexXPos + ellipseOffset * 2, startVertexYPos,
						endVertexXPos + ellipseOffset, endVertexYPos));
			}
		}
		labelEdge(g2);
		wasDragged = false;
