...
public class ApplicationController {
	...
	ISessionController duplicatedSessions = null;
	ISessionController deletingSession = null;
	ISessionController defaultGraphSession = null;
	...
	public synchronized static ApplicationController getInstance(ApplicationModel pApplicationModel) {
	...
	public void setRequestedFile(String fileName, Persistor persistor) {
		...
		ISessionController loadedSession = (ISessionController) persistor.loadFile(fileName);
		...
		if (duplicatedSessions != null) {
			appContLogger.warn("Duplicated session detected while loading. Delete duplicated session");
			sessionControllers.remove(duplicatedSessions);
			duplicatedSessions = null;
		}

		this.sessionControllers.add(loadedSession);
		applicationModel.setSession(((ISessionController) sessionControllers.lastElement()));
	}
	...
	public void deleteSession(ISessionController pSessionController) {
		...
		if (sessionControllers.size() > 0) {
			...
			applicationModel.setSession(((ISessionController) sessionControllers.firstElement()));
	 ...
	 * @param pSessionName
	 */
	public synchronized void addTreeModel(TreeModel pTreeModel, long pId, String pSessionName) {
		...
		if (isSessionExisting == false) {
			appContLogger.debug("Build new tree session");
			ITreeSessionController newSession = new TreeSessionController(pId, pSessionName, pTreeModel);
	 ...
	 * @param pSessionName
	 */
	public synchronized void addModel(GraphModel pGraphModel, long pId, String pSessionName) {
		...
		if (isSessionExisting == false) {
			...
			GraphSessionController newSession = new GraphSessionController(pId, pSessionName, pGraphModel);
			sessionControllers.add(newSession);
			appContLogger.debug("Set session as actual model");
			applicationModel.setSession(newSession);
		}
