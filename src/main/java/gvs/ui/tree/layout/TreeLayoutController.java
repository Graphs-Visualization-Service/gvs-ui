...
public class TreeLayoutController {
	...
	public void setElements(TreeModel pModel) {
		treeContLogger.info("Tree LayoutController has new elements detected, start layouting procedure");
		myModel = pModel;
	...
	private void setNodeYPositions() {
		treeContLogger.debug("setNodeYPositions(): Tree LayoutController set node positions");
		...
		while (it.hasNext()) {
			...
			levelDistance = (maxDimensionWidth / Math.pow(2, temp.getMyTreeLevel() + 1));
			treeContLogger.debug("Tree-Level: " + temp.getMyTreeLevel() + " = " + levelDistance);
			double relativePosition = (temp.getMyTreePosition() - Math.pow(2, temp.getMyTreeLevel()) + 1);
				...
				if (nodeCounter > Math.pow(2, temp.getMyTreeLevel())) {
					if (nodeCounter == temp.getMyTreePosition()) {
						if (lastFoundPosition == 0) {
							...
							relativePosition = levelDistance + 2 * levelDistance + lastFoundPosition;
	...
	private void setNodesXPosition(IBinaryNode pRoot, int pDepth, int pPosition) {
		treeContLogger.debug("Tree LayoutController is seting depth of child nodes");
		if (pRoot == null) {
