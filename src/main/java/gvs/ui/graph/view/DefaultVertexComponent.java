 ...
 *
 */
public class DefaultVertexComponent extends JComponent implements IDefaultVertexComponent {
	...
	/**
	 * Builds an instance of a vertex component. Each vertex will be displayed as a
	 * vertex component
	 * 
	 * @param pVertex
	 * @param pDim
	 ...
	 * @param pMaxPixel
	 */
	public DefaultVertexComponent(IDefaultVertex pVertex, Dimension pDim, int pLabelLength, int pMaxPixel) {
		super();
		this.vertex = pVertex;
		this.dim = pDim;
	...
	private void drawActiveVertex(Graphics2D g2) {
		GradientPaint redtowhite = new GradientPaint((int) xPosition, (int) yPosition, Color.RED,
				(int) xPosition + rectWidth, (int) yPosition, Color.WHITE);

		g2.draw(new Ellipse2D.Double(xPosition - xMarkedlineOffset + BOUNDARYXOFFSET,
				yPosition - yMarkedlineOffset + BOUNDARYYOFFSET, markedRectWidthLine, markedRectHeightLine));

		g2.setPaint(redtowhite);
		g2.fill(new Ellipse2D.Double(xPosition - xMarkedEllipseOffset + BOUNDARYXOFFSET,
				yPosition - yMarkedEllipseOffset + BOUNDARYYOFFSET, markedRectWidth, markedRectHeight));

	}

	// Draw a standard vertex component
	private void drawStandardVertex(Graphics2D g2) {
		GradientPaint vertexFillColor = new GradientPaint((int) xPosition, (int) yPosition, fillColor,
				(int) xPosition + rectWidth, (int) yPosition, Color.WHITE);

		g2.draw(new Ellipse2D.Double(xPosition - xlineOffset + BOUNDARYXOFFSET,
				yPosition - ylineOffset + BOUNDARYYOFFSET, rectWidthLine, rectHeightLine));
